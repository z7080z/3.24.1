...
//getter and setter for channel number
void 
LrWpanPhy::SetChannelNumber (uint32_t channel_number)
{
	m_channelNumber = channel_number;
}

uint32_t
LrWpanPhy::GetChannelNumber ()
{
...
//To calcalte GTS slot base on GTStxQueue
Time
LrWpanPhy::CalculateGTSTxTime (uint32_t packetSize)
{
	Time txTime = Seconds (0);
	bool isData = true;

	txTime = Seconds (GetPpduHeaderTxTime ()
                    + packetSize * 8.0 / GetDataOrSymbolRate (isData));
